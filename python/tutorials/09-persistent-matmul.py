...
@triton.jit(launch_metadata=_matmul_launch_metadata)
def matmul_kernel_tma_persistent_OVERRIDE(a_desc_ptr, b_desc_ptr, c_desc_ptr,  #
                                 M, N, K,  #
                                 BLOCK_SIZE_M: tl.constexpr,  #
                                 BLOCK_SIZE_N: tl.constexpr,  #
                                 BLOCK_SIZE_K: tl.constexpr,  #
...
def matmul_tma_persistent(a, b, c, desc_a, desc_b, desc_c, override = False, print_hash = True):
    ...
    if override:
        override_bin = matmul_kernel_tma_persistent_OVERRIDE[grid](
...
def validate(M, N, K, dtype):
    ...
    tma_persistent_result = torch.zeros((M, N), device=a.device, dtype=dtype)
    desc_a, desc_b, desc_result = tma_setup(a, b, tma_persistent_result, M, N, K)
    matmul_tma_persistent(a, b, tma_persistent_result, desc_a, desc_b, desc_result) if supports_tma() else None
    
    tma_persistent_result_OVERRIDE = torch.zeros((M, N), device=a.device, dtype=dtype)
    desc_a, desc_b, desc_result = tma_setup(a, b, tma_persistent_result_OVERRIDE, M, N, K)
    matmul_tma_persistent(a, b, tma_persistent_result_OVERRIDE, desc_a, desc_b, desc_result, override=True) if supports_tma() else None
    ...
    if tma_persistent_result is not None:
        naive_vs_tma_persistent_OVERRIDE = "✅" if torch.allclose(cublas_result.to(torch.float16),
                                                        tma_persistent_result_OVERRIDE.to(torch.float16), atol=1.0) else "❌"
    ...
    if tma_persistent_result_OVERRIDE is not None:
        print(f"TMA persistent override: {naive_vs_tma_persistent_OVERRIDE}")
